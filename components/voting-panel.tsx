    ...
    const fetchActiveMeeting = async () => {
      try {
        ...
        if (!meetingsSnapshot.empty) {
          ...
          const userHasVoted = meetingData.votes?.some((vote) => vote.userId === user.uid)

          if (userHasVoted) {
            setVotingStatus("voted")
            // Find the user's vote
            const vote = meetingData.votes.find((vote) => vote.userId === user.uid)
      ...
      <CardContent>
        {votingStatus === "locked" && (
          <div className="flex flex-col items-center justify-center py-6 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-2">Voting opens 5 minutes before the meeting starts</p>
          </div>
        )}
        ...
        {votingStatus === "unlocked" && activeMeeting && (
          <div className="space-y-4">
            <p className="text-sm">
              Who arrived first at <span className="font-medium">{activeMeeting.meetingName}</span>?
            </p>
              ...
              {participants.map((participant) => (
                <Button
                  ...
                  className={cn(
                    ...
                    selectedParticipant === participant.email && "border-green-600 bg-green-900/20",
                  )}
            ...
            <div className="bg-green-900/20 p-4 rounded-lg flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  You voted for {participants.find((p) => p.email === userVote)?.displayName || userVote}
                </p>
              </div>
            </div>

            <p className="text-xs text-muted-foreground text-center">Results will be calculated after voting ends</p>
          </div>
        )}
      </CardContent>
    </Card>
