...
export default function Navbar() {
      ...
      if (user) {
        try {
          ...
          if (userDoc.exists()) {
            ...
            const nameParts = (userData.displayName || user.displayName || "").split(" ")
  ...
  const navItems = [
    ...
    { href: "/calendar", label: "Calendar", icon: <Calendar className="h-4 w-4 mr-2" /> },
  ]
      ...
      <div className="container flex h-14 items-center justify-between">
        <div className="flex items-center space-x-2">
          <Link href="/" className="flex items-center space-x-2">
            <Clock className="h-6 w-6 text-green-600" />
            <span className="font-bold hidden sm:inline-block">H2 Early Crew</span>
          </Link>
        ...
        <div className="flex items-center space-x-4">
          {isAdmin && <div className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full">Admin</div>}
            ...
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="relative h-8 w-8 rounded-full">
                <Avatar className="h-8 w-8 border border-green-800">
                  ...
                  <AvatarFallback className="bg-green-900 text-green-300">{initials}</AvatarFallback>
                </Avatar>
              </Button>
            </DropdownMenuTrigger>
