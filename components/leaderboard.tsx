...
export function Leaderboard({ showProgress = false }) {
    ...
    const fetchLeaderboard = async () => {
      try {
        ...
        for (const userData of usersData) {
          const winnerQuery = query(collection(db, "meetings"), where("winner", "==", userData.email))
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {users.length === 0 ? (
          <div className="text-center py-6 text-muted-foreground">No data available yet</div>
        ) : (
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  ...
                  <div className="font-semibold text-green-600">{userData.wins} wins</div>
                </div>

                {showProgress && <Progress value={(userData.wins / maxWins) * 100} className="h-2 mt-2" />}
              </div>
