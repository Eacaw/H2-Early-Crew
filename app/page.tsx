...
export default function Home() {
    ...
    const fetchMetrics = async () => {
      try {
        ...
        meetingsSnapshot.forEach((doc) => {
          ...
          if (meeting.votes) {
            totalVotes += meeting.votes.length
          }
        })
        ...
        const usersWithWins = usersSnapshot.docs.map((doc) => {
          ...
          // Count wins by checking meetings where this user is the winner
          return {
            ...userData,
        ...
        for (const user of usersWithWins) {
          const winnerQuery = query(collection(db, "meetings"), where("winner", "==", user.email))
        ...
        const topWinner =
          usersWithWins.length > 0
            ? { displayName: usersWithWins[0].displayName, wins: usersWithWins[0].wins }
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-3xl font-bold tracking-tight text-white">H2 Early Crew Dashboard</h1>
